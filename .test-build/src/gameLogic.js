...
export function getTargetMoleCount(elapsedTime) {
    if (elapsedTime >= 60)
        return 5;
    if (elapsedTime >= 30)
        return 4;
...
export function getTargetSisiCount(elapsedTime, targetMoleCount) {
...
export function getSisiPhase(elapsedTime) {
    if (elapsedTime >= 120)
        return 'Disaster';
    if (elapsedTime >= 90)
        return 'Mixed';
...
export function getSisiStatus(elapsedTime, sisiIndex) {
    const phase = getSisiPhase(elapsedTime);
    if (phase === 'Regular')
        return 'sisi';
    if (phase === 'Masked')
        return 'disguised-sisi';
    return sisiIndex % 2 === 0 ? 'disguised-sisi' : 'sisi';
}
export function canShowComboHole(elapsedTime) {
    return elapsedTime >= 90;
}
export function canShowDoubleSisi(elapsedTime) {
    return elapsedTime >= 120;
}
export function getBetterScore(currentScore, pastBestScore) {
    return Math.max(currentScore, pastBestScore);
}
export function getProtectedName(inputName) {
